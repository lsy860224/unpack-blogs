...
11.1. **GSC 추적 자동 갱신 (B-9 신규):**
    - Pipeline 카드 frontmatter에 다음 4개 필드 자동 주입(없으면 추가, 있으면 유지):
      ```yaml
      gsc_submitted: false              # 사용자 GSC 제출 후 수동 true 갱신
      gsc_submitted_date: null          # 제출 시 KST 일자 기록
      gsc_indexed: false                # GSC Internal Links 보고서 인지 후 true
      gsc_indexed_check_date: null      # 마지막 인지 확인 일자
      ```
    - `docs/gsc-indexing-status.md` 의 해당 도메인 표 끝에 신규 행 자동 append:
      `| N | \`{slug}\` | {date} | ⭐⭐ | [ ] | 🔴 |`
    - 위 두 갱신은 발행 직후(커밋 전) 동시에 수행하고 같은 PR에 포함
